...
export const metadata = { title: "Packages · الباقات — Nakhla · نخلة" };
...
export default async function PackagesPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-3xl font-black">باقاتك · Your packages</h1>
      <p className="text-muted mt-1.5">حدد ما يمكن للعلامات السعودية حجزه مباشرة — بالريال السعودي.</p>
          ...
          {(me_profile.packages ?? []).map((p) => (
            <li key={p.id} className="p-4 flex items-center gap-3">
              ...
              <form action={`/api/packages/${p.id}/delete`} method="post">
                <button className="text-sm text-red-600 font-semibold">حذف · Remove</button>
              </form>
            </li>
          ))}
          {(me_profile.packages ?? []).length === 0 && (
            <li className="p-8 text-center text-sm text-muted">لا توجد باقات بعد — أضف واحدة أدناه.</li>
          )}
        </ul>
      </section>

      <section className="mt-6 rounded-2xl border border-border bg-elevated p-6">
        <h2 className="font-bold text-lg">أضف باقة · Add a package</h2>
        <form action="/api/packages/create" method="post" className="mt-4 grid sm:grid-cols-2 gap-3">
          <label className="block sm:col-span-2">
            <span className="text-xs font-semibold">العنوان · Title</span>
            <input name="title" required placeholder="ريل واحد على إنستغرام · 1 Instagram Reel" className="mt-1 w-full px-3.5 py-3 rounded-xl border border-border bg-elevated" />
          </label>
          <label className="block">
            <span className="text-xs font-semibold">السعر (ر.س) · Price (SAR)</span>
          ...
          <label className="block">
            <span className="text-xs font-semibold">مدة التسليم (أيام) · Delivery (days)</span>
          ...
          <label className="block sm:col-span-2">
            <span className="text-xs font-semibold">الوصف · Description</span>
            <textarea name="description" rows={3} className="mt-1 w-full px-3.5 py-3 rounded-xl border border-border bg-elevated" />
          </label>
          <button className="sm:col-span-2 px-5 py-3 rounded-xl brand-gradient text-white font-bold">أضف الباقة · Add package</button>
        </form>
      </section>
    </div>
  );
