  ...
  return (
    <header className="flex items-end justify-between gap-4 mb-6">
      <div>
        ...
        {subtitle && <p className="text-sm text-[#8b8ba0] mt-1">{subtitle}</p>}
      </div>
      {action}
    </header>
  );
...
export function StatGrid({ stats }: { stats: { label: string; value: string; sub?: string; tone?: "success" | "danger" | "warn" }[] }) {
  const tone = { success: "#10b981", danger: "#ef4444", warn: "#f59e0b" } as const;
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {stats.map((s) => (
        <div key={s.label} className="card p-4">
          <p className="text-[10px] uppercase tracking-wider text-[#8b8ba0]">{s.label}</p>
          <p className="text-2xl font-black mt-1" style={s.tone ? { color: tone[s.tone] } : undefined}>{s.value}</p>
          {s.sub && <p className="text-xs text-[#8b8ba0] mt-1">{s.sub}</p>}
        </div>
      ))}
    </div>
  );
...
export function Card({ title, action, children, padding = true }: { title?: string; action?: React.ReactNode; children: React.ReactNode; padding?: boolean }) {
  return (
    <section className="card">
      {title && (
        <header className="flex items-center justify-between px-5 py-4 border-b border-[#1f1f30]">
...
export function EmptyState({ title, body, cta }: { title: string; body: string; cta?: { href: string; label: string } }) {
  return (
    <div className="text-center py-16">
      ...
      <p className="text-[#8b8ba0] text-sm mt-1">{body}</p>
