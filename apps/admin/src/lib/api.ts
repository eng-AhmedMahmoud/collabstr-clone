...
type FetchOpts = RequestInit & { cookieHeader?: string };
...
export async function serverApi() {
  ...
  return {
    ...
    patch: <T>(path: string, body?: unknown) =>
      call<T>(path, { method: "PATCH", cookieHeader, body: body ? JSON.stringify(body) : undefined }),
    del: <T>(path: string) => call<T>(path, { method: "DELETE", cookieHeader }),
  };
}
